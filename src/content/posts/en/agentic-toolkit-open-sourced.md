...
What's public right now is the first real slice, not just a teaser skeleton: the marketplace manifest, MIT, DCO sign-off, CI, contributor scaffolding, and one fully documented plugin, Feinschliff. That plugin already carries the release logic in miniature: v2 template architecture, 34 baked PowerPoint templates per brand, 11 openly licensed brand packs, and on the current branch, a slide-pattern atlas that starts grounding planning in concrete exemplars. Twenty-one plugins still exist on the private side. They've been built and battle-tested across actual daily work: research, knowledge management, deck production, video pipelines, homelab control, social amplification. They'll arrive here one by one, each as its own teaching artifact, each in the shape it deserves.
